...
BTN_TODAY, BTN_TOMORROW, BTN_MONTH, BTN_REGION, BTN_DUA, BTN_CHANNEL = (
...
def start(update, context):
    ...
    update.message.reply_html(
        '🤖 Assalomu aleykum <b>{}!</b>\n \n<b>Ramazon oyi muborak bo\'sin 😊</b>\n \nSizga qaysi mintaqa bo\'yicha malumot kerak ⁉️'.
            format(user.first_name, user.last_name), reply_markup=InlineKeyboardMarkup(buttons))
...
def calendar_today(update, context):
    try:
        my_image = Image.open("images/image_temp.png")
        title_font = ImageFont.truetype('Mulish_wght.ttf', 20)
        title_text = "Bugun {}".format(str(datetime.now().date()))
        image_editable = ImageDraw.Draw(my_image)
        image_editable.text((100,15), title_text, (237, 230, 211), font=title_font)
...
def calendar_tomorrow(update, context):
    try:
        my_image = Image.open("images/image_temp.png")
        title_font = ImageFont.truetype('Mulish_wght.ttf', 20)
        title_text = "Ertaga {}\n".format( str(datetime.now().date() + timedelta(days=1)))
        image_editable = ImageDraw.Draw(my_image)
        image_editable.text((200,450), title_text, (300, 230, 211), font=title_font)
        my_image.save('images/img.jpg')
        user_id = update.message.from_user.id
        if not user_region[user_id]:
            return STATE_REGION
...
def calendar_month(update, context):
    ...
    region_id = user_region[user_id]
    region = db.get_region(region_id)
    photo_path = 'images/table/region_{}.png'.format(region['id'])
...
def select_dua(update, context):
    my_image = Image.open("images/dua_temp.png")
    title_font = ImageFont.truetype('OrelegaOne-Regular.ttf', 55)
    title_text = "t.me/att_tavba"
    image_editable = ImageDraw.Draw(my_image)
    image_editable.text((50,120), title_text, (237, 230, 211), font=title_font)
    my_image.save('images/img_dua.png')
    ...
    update.message.reply_photo(photo=open('images/img_dua.png', 'rb'),
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            STATE_REGION: [
                ...
                MessageHandler(Filters.regex('^(' + BTN_CHANNEL + ')$'), select_channel)
